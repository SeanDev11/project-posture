...
//  PPnoStoryboards
//
//  Created by Sean Devine on 22.08.19.
...
class popUpTA: UIViewController {
    ...
    func setValues() {
        ...
        aptCheckbox.setOn(defaults.bool(forKey: "AnteriorPelvicTilt"), animated: false)
        rsCheckbox.setOn(defaults.bool(forKey: "RoundedShoulders"), animated: false)
        fnCheckbox.setOn(defaults.bool(forKey: "ForwardNeck"), animated: false)
    }
...
//        view.addConstraintsWithFormat(format: "V:|-100-[v0]-100-|", views: popUp)
    ...
    let aptLabel: UILabel = {
        let screenH = UIScreen.main.bounds.height
        let label = UILabel()
        label.text = "Anterior Pelvic Tilt"
        var fontS: CGFloat = 20
        if screenH > 850 {
            fontS = 24
        } else if screenH > 700 {
    ...
    @objc func saveChanges() {
      ...
      defaults.set(aptCheckbox.isOn, forKey: "AnteriorPelvicTilt")
       defaults.set(rsCheckbox.isOn, forKey: "RoundedShoulders")
        defaults.set(fnCheckbox.isOn, forKey: "ForwardNeck")
        
        let theMaxTime = findMaxTime() / 60
        defaults.set(theMaxTime, forKey: "TimeLimit")
