...
class MainBarController: UIViewController {
    ...
    let barView: MacawChartView = {
        let view = MacawChartView(frame: CGRect(x: 0, y: 0, width: 0, height: 0))
    ...
    let button: UIButton = {
       let btn = UIButton()
        ...
        btn.addTarget(self, action: #selector(makeGraph), for: .touchUpInside)
    ...
    let nextButton: UIButton = {
        ...
        btn.setTitle("PP implementation", for: .normal)
        btn.setTitleColor(UIColor.gray, for: .normal)
        return btn
    }()
    ...
    @objc func nextOne() {
        ...
        let finalGraph = FinalGraph()
        
        self.present(finalGraph, animated: true)
